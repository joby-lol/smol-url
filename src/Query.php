...
readonly class Query implements Stringable
{
    ...
    public function __construct(array $args = [])
    {
        ...
        foreach ($args as $key => $value) {
            if (!is_string($key)) // @phpstan-ignore-line we do want to check this at runtime
                throw new QueryException('Query keys must be strings');
            if (is_array($value)) // @phpstan-ignore-line we do want to check this at runtime
                throw new QueryException('Invalid query value type: array');
            ...
            if (!is_scalar($value)) { // @phpstan-ignore-line we do want to check this at runtime
                throw new QueryException('Invalid query value type: ' . gettype($value));
            }
            $built_args[$key] = (string)$value;
        }
        ksort($built_args);
